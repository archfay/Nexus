...
class CustomTelegramClient(TelegramClient):
    def __init__(self, *args, **kwargs):
        ...
        self._nexus_entity_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordEntity,
        ] = {}

        self._nexus_perms_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordPerms,
        ] = {}

        self._nexus_fullchannel_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordFullChannel,
        ] = {}

        self._nexus_fulluser_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordFullUser,
        ] = {}
    ...
    @property
    def nexus_entity_cache(self) -> typing.Dict[int, CacheRecordEntity]:
        return self._nexus_entity_cache

    @property
    def nexus_perms_cache(self) -> typing.Dict[int, CacheRecordPerms]:
        return self._nexus_perms_cache

    @property
    def nexus_fullchannel_cache(self) -> typing.Dict[int, CacheRecordFullChannel]:
        return self._nexus_fullchannel_cache

    @property
    def nexus_fulluser_cache(self) -> typing.Dict[int, CacheRecordFullUser]:
        return self._nexus_fulluser_cache
    ...
    async def get_entity(
        ...
        _nexus_client_id_logging_tag = copy.copy(self.tg_id)  # noqa: F841
        ...
        if (
            not force
            and hashable_entity
            and hashable_entity in self._nexus_entity_cache
            and (
                not exp
                or self._nexus_entity_cache[hashable_entity].ts + exp > time.time()
            )
        ):
            logger.debug(
                "Using cached entity %s (%s)",
                entity,
                type(self._nexus_entity_cache[hashable_entity].entity).__name__,
            )
            return copy.deepcopy(self._nexus_entity_cache[hashable_entity].entity)

        resolved_entity = await super().get_entity(entity)

        if resolved_entity:
            cache_record = CacheRecordEntity(hashable_entity, resolved_entity, exp)
            self._nexus_entity_cache[hashable_entity] = cache_record
            ...
            if getattr(resolved_entity, "id", None):
                ...
                self._nexus_entity_cache[resolved_entity.id] = cache_record
            ...
            if getattr(resolved_entity, "username", None):
                ...
                self._nexus_entity_cache[f"@{resolved_entity.username}"] = cache_record
                self._nexus_entity_cache[resolved_entity.username] = cache_record
    ...
    async def get_perms_cached(
        ...
        if (
            ...
            and hashable_entity
            and hashable_user
            and hashable_user in self._nexus_perms_cache.get(hashable_entity, {})
            and (
                not exp
                or self._nexus_perms_cache[hashable_entity][hashable_user].ts + exp
                > time.time()
            )
        ):
            logger.debug("Using cached perms %s (%s)", hashable_entity, hashable_user)
            return copy.deepcopy(
                self._nexus_perms_cache[hashable_entity][hashable_user].perms
            )
        ...
        if resolved_perms:
            ...
            self._nexus_perms_cache.setdefault(hashable_entity, {})[
                hashable_user
            ] = cache_record
            ...
            def save_user(key: typing.Union[str, int]):
                ...
                if getattr(user, "id", None):
                    self._nexus_perms_cache.setdefault(key, {})[user.id] = cache_record

                if getattr(user, "username", None):
                    self._nexus_perms_cache.setdefault(key, {})[
                        f"@{user.username}"
                    ] = cache_record
                    self._nexus_perms_cache.setdefault(key, {})[
                        user.username
    ...
    async def get_fullchannel(
        ...
        if (
            not force
            and self._nexus_fullchannel_cache.get(hashable_entity)
            and not self._nexus_fullchannel_cache[hashable_entity].expired
            and self._nexus_fullchannel_cache[hashable_entity].ts + exp > time.time()
        ):
            return self._nexus_fullchannel_cache[hashable_entity].full_channel

        result = await self(GetFullChannelRequest(channel=entity))
        self._nexus_fullchannel_cache[hashable_entity] = CacheRecordFullChannel(
            hashable_entity,
            result,
            exp,
        )
    ...
    async def get_fulluser(
        ...
        if (
            not force
            and self._nexus_fulluser_cache.get(hashable_entity)
            and not self._nexus_fulluser_cache[hashable_entity].expired
            and self._nexus_fulluser_cache[hashable_entity].ts + exp > time.time()
        ):
            return self._nexus_fulluser_cache[hashable_entity].full_user

        result = await self(GetFullUserRequest(entity))
        self._nexus_fulluser_cache[hashable_entity] = CacheRecordFullUser(
            hashable_entity,
            result,
            exp,
        )
