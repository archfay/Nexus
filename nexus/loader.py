...
class InfiniteLoop:
    ...
    def stop(self, *args, **kwargs):
        with contextlib.suppress(AttributeError):
            _nexus_client_id_logging_tag = copy.copy(  # noqa: F841
...
def tag(*tags, **kwarg_tags):
    ...
    @loader.tag("only_messages", "only_pm", regex=r"^[.] ?nexus$", from_id=659800858)
...
class Modules:
    ...
    def __init__(
        ...
        self.client.nexus_inline = self.inline
    ...
    def register_commands(self, instance: Module):
        ...
        if instance.__origin__.startswith("<core"):
            self._core_commands += list(
                map(lambda x: x.lower(), list(instance.nexus_commands))
            )

        for _command, cmd in instance.nexus_commands.items():
    ...
    def register_inline_stuff(self, instance: Module):
        ...
        for name, func in instance.nexus_callback_handlers.copy().items():
    ...
    def register_watchers(self, instance: Module):
        ...
        for _watcher in instance.nexus_watchers.values():
