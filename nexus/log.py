...
def getlines(filename: str, module_globals=None) -> str:
    ...
    source code of Nexus modules properly. This is needed for
    ...
    try:
        if filename.startswith("<") and filename.endswith(">"):
            ...
            if (module.startswith("nexus.modules")) and module in sys.modules:
                return list(
                    map(
                        lambda x: f"{x}\n",
...
class TelegramLogsHandler(logging.Handler):
    ...
    def install_tg_log(self, mod: Module):
        ...
        # store module reference and read per-module logging settings
        self._mods[mod.tg_id] = mod
        self._mods_settings[mod.tg_id] = {
            "sanitize_logs": bool(mod.db.get(__name__, "sanitize_logs", True)),
            "minimize_tg": bool(mod.db.get(__name__, "minimize_tg", False)),
            "tg_level": int(mod.db.get(__name__, "tg_level", self.tg_level)),
        }
    ...
    def dumps(
        ...
        return [
            ...
            and (not record.nexus_caller or client_id == record.nexus_caller)
        ]
    ...
    async def _show_full_trace(
        ...
        chunks = list(utils.smart_split(*herokutl.extensions.html.parse(msg), 4096))
    ...
    async def sender(self):
        async with self._send_lock:
            # build per-client textual queues with sensitive data redacted
            self._queue = {
                client_id: utils.chunks(
                    utils.escape_html(
                        ...
                        if self._mods_settings.get(client_id, {}).get("sanitize_logs", True)
                        else "".join(
                            [
                                item[0]
                                for item in self.tg_buff
    ...
    def emit(self, record: logging.LogRecord):
        try:
            caller = next(
                (
                    frame_info.frame.f_locals["_nexus_client_id_logging_tag"]
                    for frame_info in inspect.stack()
                    if isinstance(
                        getattr(getattr(frame_info, "frame", None), "f_locals", {}).get(
                            "_nexus_client_id_logging_tag"
                        ),
                        int,
                    )
                ),
        ...
        record.nexus_caller = caller

        # use per-client tg_level or fallback to global
        effective_tg_level = (
            self._mods_settings.get(caller, {}).get("tg_level", self.tg_level)
            if caller
            else self.tg_level
        )

        if record.levelno >= effective_tg_level:
            if record.exc_info:
                try:
                    if record.args:
