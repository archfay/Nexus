...
class CommandDispatcher:
    def __init__(
        ...
        self._me = self._client.nexus_me.id
        self._cached_usernames = [
            (
                self._client.nexus_me.username.lower()
                if self._client.nexus_me.username
                else str(self._client.nexus_me.id)
            )
        ]

        self._cached_usernames.extend(
            u.username.lower()
            for u in getattr(self._client.nexus_me, "usernames", []) or []
        )
    ...
    async def handle_incoming(
        ...
        try:
            ...
            logger.debug("Can't get chat_id, skipping message")
            return
    ...
    async def _external_bl_reload_loop(self):
        while True:
            ...
            self._modules.watchers = self._modules.get_watchers()
