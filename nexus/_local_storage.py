...
class LocalStorage:
    ...
    def __init__(self):
        self._path = os.path.join(os.path.expanduser("~"), ".nexus", "modules_cache")
        self._ensure_dirs()
    ...
    async def fetch(self, url: str, auth: typing.Optional[str] = None) -> str:
        ...
        try:
            r = await utils.run_sync(
                ...
                headers={
                    "User-Agent": "Nexus Userbot",
                    "X-Nexus-Version": ".".join(map(str, __version__)),
                    "X-Nexus-Commit-SHA": utils.get_git_hash(),
                    "X-Nexus-User": str(self._client.tg_id),
                },
            )
            r.raise_for_status()
        except Exception:
