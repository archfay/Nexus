...
class NexusBackupMod(loader.Module):
    ...
    async def handler(self):
        try:
            ...
            db = io.BytesIO(utils.json_dumps(self._db).encode())
            db.name = "db.json"
            ...
            with zipfile.ZipFile(mods, "w", zipfile.ZIP_DEFLATED) as zipf:
                ...
                zipf.writestr(
                    "db_mods.json",
                    utils.json_dumps(self.lookup("Loader").get("loaded_modules", {})),
                )
                ...
                reply_markup=self.inline.generate_markup(
                    [
                        [
                            {
                                "text": "↪️ Restore this",
                                "data": "nexus/backupall/restore/confirm",
                            }
                        ]
                    ]
                ),
    ...
    async def restore(self, call: BotInlineCall):
        if not call.data.startswith("nexus/backupall/restore"):
            return

        if call.data == "nexus/backupall/restore/confirm":
            await utils.answer(
                call,
                "❓ <b>Are you sure?</b>",
                reply_markup={
                    "text": "✅ Yes",
                    "data": "nexus/backupall/restore",
                },
            )
            return
        ...
        try:
            ...
            with zipfile.ZipFile(zipfile_bytes) as zf:
                with zf.open("db.json") as f:
                    db_data = utils.json_loads(f.read().decode())

                with contextlib.suppress(KeyError):
                    db_data["nexus.inline"].pop("bot_token")
                ...
                with zf.open("mods.zip") as modzip_bytes:
                    with zipfile.ZipFile(io.BytesIO(modzip_bytes.read())) as modzip:
                        with modzip.open("db_mods.json", "r") as modules:
                            db_mods = utils.json_loads(modules.read().decode())
    ...
    def _convert(self, backup):
        fixed = re.sub(r"(hikka\.)(\S+\":)", lambda m: "nexus." + m.group(2), backup)
    ...
    async def backupdb(self, message: Message):
        txt = io.BytesIO(utils.json_dumps(self._db).encode())
    ...
    async def restoredb(self, message: Message):
        ...
        try:
            decoded_text = utils.json_loads(file.decode())
        ...
        with contextlib.suppress(KeyError):
            decoded_text["nexus.inline"].pop("bot_token")
    ...
    async def backupmods(self, message: Message):
        ...
        db_mods = utils.json_dumps(self.lookup("Loader").get("loaded_modules", {})).encode()
