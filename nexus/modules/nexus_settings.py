...
@loader.tds
class NexusSettingsMod(loader.Module):
    """Advanced settings for Nexus Userbot"""

    strings = {"name": "NexusSettings"}
    ...
    async def _uninstall(self, call: InlineCall):
        ...
        async for dialog in self._client.iter_dialogs(
            ...
            if (
                ...
                and (
                    ...
                    and dialog.name in {"nexus-logs", "silent-tags"}
                )
                or (
        ...
        if any(folder.title == "nexus" for folder in folders):
            folder_id = max(
                folders,
                key=lambda x: x.id,
            ).id
    ...
    @loader.command()
    async def uninstall_nexus(self, message: Message):
    ...
    async def invokecmd(self, message: Message):
        ...
        if module == "core":
            if method == "flush_entity_cache":
                result = (
                    f"Dropped {len(self._client._nexus_entity_cache)} cache records"
                )
                self._client._nexus_entity_cache = {}
            elif method == "flush_fulluser_cache":
                result = (
                    f"Dropped {len(self._client._nexus_fulluser_cache)} cache records"
                )
                self._client._nexus_fulluser_cache = {}
            elif method == "flush_fullchannel_cache":
                result = (
                    f"Dropped {len(self._client._nexus_fullchannel_cache)} cache"
                    " records"
                )
                self._client._nexus_fullchannel_cache = {}
            elif method == "flush_perms_cache":
                result = f"Dropped {len(self._client._nexus_perms_cache)} cache records"
                self._client._nexus_perms_cache = {}
                ...
                result = (
                    f"Dropped {len(self._client._nexus_entity_cache)} entity cache"
                    " records\nDropped"
                    f" {len(self._client._nexus_fulluser_cache)} fulluser cache"
                    " records\nDropped"
                    f" {len(self._client._nexus_fullchannel_cache)} fullchannel cache"
                    " records\nDropped"
                ...
                self._client._nexus_fulluser_cache = {}
                self._client._nexus_fullchannel_cache = {}
                self._client.nexus_me = await self._client.get_me()
                ...
                result = (
                    "Entity cache:"
                    f" {len(self._client._nexus_entity_cache)} records\nFulluser cache:"
                    f" {len(self._client._nexus_fulluser_cache)} records\nFullchannel"
                    " cache:"
                    f" {len(self._client._nexus_fullchannel_cache)} records\nLoader"
