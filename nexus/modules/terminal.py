...
class SudoMessageEditor(MessageEditor):
    ...
    async def update_stderr(self, stderr):
        ...
        if any(lastlines[0] == i for i in self.PASS_REQ) and self.state == 0:
            ...
            text = self.strings("auth_needed").format(self.message.client.nexus_me.id)
...
class TerminalMod(loader.Module):
    ...
    async def run_command(
        ...
        # attach start time for execution time reporting
        try:
            editor.start_time = time.perf_counter()
        except Exception:
            editor.start_time = None
