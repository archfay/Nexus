...
class Evaluator(loader.Module):
    ...
    async def e(self, message: Message):
        ...
        with contextlib.suppress(MessageIdInvalidError):
            ...
            try:
                out_msg += f"\n\n<i>Execution time: {duration_ms} ms</i>"
            except Exception:
                pass

            await utils.answer(message, out_msg)
    ...
    async def enode(self, message: Message):
        ...
        with contextlib.suppress(MessageIdInvalidError):
            out = self.strings("err" if error else "eval").format(
                "4985643941807260310",
                "javascript",
                utils.escape_html(code),
                "error" if error else "output",
    ...
    def censor(self, ret: str) -> str:
        ret = ret.replace(str(self._client.nexus_me.phone), "&lt;phone&gt;")
        ...
        if btoken := self._db.get("nexus.inline", "bot_token", False):
            ret = ret.replace(
                btoken,
                f'{btoken.split(":")[0]}:{"*" * 26}',
            )
