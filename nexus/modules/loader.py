...
class LoaderMod(loader.Module):
    ...
    async def load_module(
        ...
        if re.search(r"# ?scope: ?nexus_min", doc):
            ver = re.search(r"# ?scope: ?nexus_min ((?:\d+\.){2}\d+)", doc).group(1)
        ...
        module_name = f"nexus.modules.{uid}"
        doc = geek.compat(doc)
        ...
        try:
            try:
                ...
                async def inner_proxy():
                    ...
                    while True:
                        if hasattr(instance, "nexus_wait_channel_approve"):
                            if message:
                                (
                                    module,
                                    channel,
                                    reason,
                                ) = instance.nexus_wait_channel_approve
                                message = await utils.answer(
                                    message,
        ...
        try:
            developer_entity = await (
                ...
                if (
                    developer in self._client.nexus_entity_cache
                    and getattr(
