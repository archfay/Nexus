...
class UpdaterMod(loader.Module):
    """Updates itself, tracks latest Nexus releases, and notifies you, if update is required"""
            ...
            if not self.config["autoupdate"]:
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        r = await session.get(
                            url=f"https://api.github.com/repos/DoNotWeb/Nexus/contents/nexus/version.py?ref={version.branch}",
    ...
    async def update_call(self, call: InlineCall):
        ...
        if call.data not in {"nexus/update", "nexus/ignore_upd"}:
            return

        if call.data == "nexus/ignore_upd":
    ...
    async def client_ready(self):
        ...
        self._markup = lambda: self.inline.generate_markup(
            [
                {"text": self.strings("update"), "data": "nexus/update"},
                {"text": self.strings("ignore"), "data": "nexus/ignore_upd"},
            ]
        )
    ...
    async def _add_folder(self):
        ...
        if any(getattr(folder, "title", None) == "nexus" for folder in folders.filters):
            return
        ...
        try:
            await self._client(
                UpdateDialogFilterRequest(
                    ...
                    DialogFilter(
                        folder_id,
                        title=TextWithEntities(text="Nexus", entities=[]),
                        pinned_peers=(
                            [
            ...
            logger.critical(
                "Can't create Nexus folder. Possible reasons are:\n"
