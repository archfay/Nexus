...
@loader.tds
class NexusWebMod(loader.Module):
    """Web/Inline mode add account"""

    strings = {"name": "NexusWeb"}
    ...
    async def weburl(self, message: Message, force: bool = False):
        ...
        if "OTHERHOST" in os.environ or "JAMHOST" in os.environ:
            ...
            if not main.nexus.web:
                main.nexus.web = core.Web(
                    data_root=main.BASE_DIR,
                    api_token=main.nexus.api_token,
                    proxy=main.nexus.proxy,
                    connection=main.nexus.conn,
                )
                await main.nexus.web.add_loader(self._client, self.allmodules, self._db)
                await main.nexus.web.start_if_ready(
                    len(self.allclients),
                    main.nexus.arguments.port,
                    proxy_pass=main.nexus.arguments.proxy_pass,
                )
            ...
            url = await main.nexus.web.get_url(proxy_pass=True)
    ...
    def _get_client(self) -> CustomTelegramClient:
        return CustomTelegramClient(
            MemorySession(),
            main.nexus.api_token.ID,
            main.nexus.api_token.HASH,
            connection=main.nexus.conn,
            proxy=main.nexus.proxy,
    ...
    async def schedule_restart(self, call, client):
        ...
        await main.nexus.save_client_session(client, delay_restart=False)
        restart()
