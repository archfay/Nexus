...
class Quickstart(loader.Module):
    ...
    async def client_ready(self):
        ...
        self.text = (
            lambda: (
                f"<b>Prefix: {utils.escape_html(self.get_prefix())}</b>\n\n"
                + self.strings("base")
                + ("\n" + ((self.strings("lavhost") if "LAVHOST" in os.environ else ""))).rstrip()
            )
        )
    ...
    async def lang(self, call: BotInlineCall):
        if not call.data.startswith("nexus/lang/"):
            return
