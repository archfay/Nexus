...
class NexusInfoMod(loader.Module):
    ...
    def remove_emoji_and_html(self, text: str) -> str:
        ...
        clean_text = "".join(
            [str for str in text if not any(i in str for i in emoji_list)]
        )
        return clean_text
    ...
    def _render_info(self, start: float) -> str:
        ...
        me = (
            self.config["imgSettings"][0]
            if (self.config["imgSettings"][0] != "Лапокапканот")
            and self.config["switchInfo"]
            else '<b><a href="tg://user?id={}">{}</a></b>'.format(
                self._client.nexus_me.id,
                utils.escape_html(get_display_name(self._client.nexus_me)),
            )
            .replace("{", "")
            .replace("}", "")
        )
        ...
        return (
            ...
            if self.config["custom_message"]
                ...
                (
                    ...
                    if self._client.nexus_me.premium and self.config["show_nexus"]
                    else ""
                ),
    ...
    def _get_info_photo(self, start: float) -> Optional[Path]:
        ...
        # normalize imgset to expected length and types to avoid IndexError/TypeError
        defaults = ["Лапокапканот", 30, "#000", "0|0", "mm", 0, "#000"]
        if not isinstance(imgset, (list, tuple)):
            imgset = defaults
        else:
            imgset = list(imgset)
            for i in range(len(defaults)):
                if i >= len(imgset) or imgset[i] is None:
                    imgset[i] = defaults[i]
        if imgform in ["jpg", "jpeg", "png", "bmp", "webp"]:
            response = requests.get(
                (
                    self.config["banner_url"]
                    if not self.config["banner_url"].startswith("https://imgur")
                    else self.imgur(self.config["banner_url"])
                ),
                stream=True,
                headers={
                    "User-Agent": (
            ...
            font_paths = glob.glob(f"{os.getcwd()}/assets/font.*")
            if font_paths:
                try:
                    font = ImageFont.truetype(font_paths[0], size=utils.safe_int(imgset[1], 30), encoding="unic")
                except Exception:
            ...
            # safe parsing for stroke width/fill (users may misconfigure imgSettings)
            stroke_w = utils.safe_int(imgset[5], 0)

            stroke_f = imgset[6] if isinstance(imgset[6], str) and imgset[6].startswith("#") else "#000"

            coords = utils.safe_coords(imgset[3], w, h)
            anchor = imgset[4] if isinstance(imgset[4], str) and len(imgset[4]) <= 2 else "mm"
            fill = utils.safe_color(imgset[2], "#000")
