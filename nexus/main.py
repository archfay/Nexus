...
def get_config_key(key: str) -> typing.Union[str, bool]:
    ...
    """
    try:
        return utils.json_loads(CONFIG_PATH.read_text()).get(key, False)
...
def save_config_key(key: str, value: str) -> bool:
    ...
    try:
        ...
        config = utils.json_loads(CONFIG_PATH.read_text())
    except FileNotFoundError:
    ...
    config[key] = value
    # And save config
    CONFIG_PATH.write_text(utils.json_dumps(config, indent=4))
    return True
...
class Nexus:
    ...
    def _read_sessions(self):
        ...
        self.sessions += [
            ...
            for session in filter(
                lambda f: (f.startswith("nexus-") or f.startswith("hikka-"))
    ...
    async def save_client_session(
        ...
        *,
        ...
        client.nexus_db = database.Database(client)
        await client.nexus_db.init()
    ...
    async def _phone_login(self, client: CustomTelegramClient) -> bool:
        ...
        while bot := input(
            "You can enter a custom bot username or leave it empty and Nexus will generate a random one: "
        ):
            try:
                if await self._check_bot(client, bot):
                    db.set("nexus.inline", "custom_bot", bot)
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        ...
                        if response.status == 200:
                            ...
                            logging.debug(
                                f"Could not load allowed beta testers ids: {response.status}"
                            )
            except Exception:
                logging.debug("Failed to fetch allowed beta testers ids", exc_info=True)
    ...
    async def _badge(self, client: CustomTelegramClient):
        ...
        try:
            ...
            logging.debug(
                ...
                client.tg_id,
                client.nexus_db.get(__name__, "command_prefix", False) or ".",
            )
        except Exception:
