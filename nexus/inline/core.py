...
class InlineManager(
    ...
    def __init__(
        ...
        self._token = db.get("nexus.inline", "bot_token", False)
    ...
    async def register_manager(
        ...
        self._name = get_display_name(self._client.nexus_me)
        ...
        try:
            m = await self._client.send_message(self.bot_username, "/start nexus init")
            ...
            try:
                m = await self._client.send_message(
                    self.bot_username, "/start nexus init"
                )
            except Exception:
        ...
        async def new(*args, **kwargs):
            ...
            try:
                ...
                logger.critical("Got Unauthorized while polling, revoking token")
                try:
                    await revoke()
                except Exception:
                    logger.exception("Failed to revoke token on unauthorized")
