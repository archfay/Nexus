...
class TokenObtainment(InlineUnit):
    async def _create_bot(self):
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            if self._db.get("nexus.inline", "custom_bot", False):
                username = self._db.get("nexus.inline", "custom_bot").strip("@")
                username = f"@{username}"
                try:
            ...
            try:
                ...
                m = await conv.send_file(f"{os.getcwd()}/assets/nexus.png")
                r = await conv.get_response()
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    ...
                    if not self._db.get(
                        ...
                        "custom_bot",
                        False,
                    ) and not re.search(r"@nexus_[0-9a-zA-Z]{6}_bot", button.text):
                        continue
                    ...
                    self._db.set("nexus.inline", "bot_token", token)
                    self._token = token
