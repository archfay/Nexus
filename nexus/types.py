...
class Module:
    ...
    @property
    def nexus_commands(self) -> typing.Dict[str, Command]:
        """List of commands that module supports"""
        return get_commands(self)
    ...
    @property
    def nexus_inline_handlers(self) -> typing.Dict[str, Command]:
        """List of inline handlers that module supports"""
        return get_inline_handlers(self)
    ...
    @property
    def nexus_callback_handlers(self) -> typing.Dict[str, Command]:
        """List of callback handlers that module supports"""
        return get_callback_handlers(self)
    ...
    @property
    def nexus_watchers(self) -> typing.Dict[str, Command]:
        """List of watchers that module supports"""
        return get_watchers(self)
    ...
    @nexus_commands.setter
    def nexus_commands(self, _):
        pass
    ...
    @nexus_inline_handlers.setter
    def nexus_inline_handlers(self, _):
        pass
    ...
    @nexus_callback_handlers.setter
    def nexus_callback_handlers(self, _):
        pass
    ...
    @nexus_watchers.setter
    def nexus_watchers(self, _):
        pass
    ...
    async def _decline(
        ...
        self._db.set(
            "nexus.main",
            "declined_joins",
            list(set(self._db.get("nexus.main", "declined_joins", []) + [channel.id])),
        )
        event.status = False
        event.set()
        await call.edit(
    ...
    async def request_join(
        ...
        if channel.id in self._db.get("nexus.main", "declined_joins", []):
            if assure_joined:
                raise LoadError(
        ...
        with contextlib.suppress(AttributeError):
            delattr(self, "nexus_wait_channel_approve")
    ...
    async def import_lib(
        ...
        *,
        ...
        if re.search(r"# ?scope: ?nexus_min", code):
            ver = tuple(
                map(
                    int,
                    re.search(r"# ?scope: ?nexus_min ((\d+\.){2}\d+)", code)[1].split(
                        "."
                    ),
                )
            )
            ...
            if version.__version__ < ver:
                _raise(
                    RuntimeError(
                        f"Library requires Nexus version {'{}.{}.{}'.format(*ver)}+"
                    )
                )

        module = f"nexus.libraries.{url.replace('%', '%%').replace('.', '%d')}"
        ...
        if (
            ...
            and self._db.get("nexus.main", "stats", True)
            and url is not None
